...
public protocol BleDataSyncDelegate:AnyObject {
    ...
    func onSyncGnss(_ gnsses: [GnssDataModel])
    ...
    //hash:boolean value condition use to reset device
    func onSyncUserHash(_ hash: UInt32)
    func onSyncDeviceInfo(_ deviceBaseInfo:DeviceBaseInfo)
                ...
                if let characteristics = service.characteristics {
                    for cha in characteristics {
                        if cha.uuid.uuidString == PROBUFF_CHARACT_SET_UUID {
                            ...
                            break
                        } else if cha.uuid.uuidString == PEDOMETER_NEW_CHARACT_SET_UUID {
                            connectedPeripheral?.writeValue(data, for: cha, type: .withoutResponse)
                            break
                        }
                    }
...
extension BleManager:CBPeripheralDelegate {
    ...
    public func peripheral(_ peripheral: CBPeripheral, didDiscoverCharacteristicsFor service: CBService, error: Error?) {
        ...
        if service.uuid.uuidString == PROBUFF_SERVICE_LONG_UUID {
            CmdTool.sharedInstance.setType(type: .probuffType)
        ...
        //MTK
        if service.uuid.uuidString == PEDOMETER_NEW_SERVICE_UUID {
            CmdTool.sharedInstance.setType(type: .MTKType)
            for cha in (service.characteristics ?? []) as [CBCharacteristic] {
                if cha.uuid.uuidString == PEDOMETER_NEW_CHARACT_TEST || cha.uuid.uuidString == PEDOMETER_OLD_CHARACT_TEST  {
                    peripheral.setNotifyValue(true, for: cha)
                } else if cha.uuid.uuidString == PEDOMETER_NEW_CHARACT_SET_UUID  {
                    writeCharacteristic = cha
                }
    ...
    public func peripheral(_ peripheral: CBPeripheral, didUpdateValueFor characteristic: CBCharacteristic, error: Error?) {
        if let data = characteristic.value {
            if characteristic.uuid.uuidString == PROBUFF_CHARACT_READ_UUID {
                //Probuff 数据处理
                ProbuffDataHandle.sharedInstance.braceletCmdReceive(bytes: data)
                
            } else if characteristic.uuid.uuidString ==  PEDOMETER_NEW_CHARACT_TEST || characteristic.uuid.uuidString ==  PEDOMETER_OLD_CHARACT_TEST {
                //MTK 数据处理
                MTKDataHandle.sharedInstance.braceletCmdReceive(bytes: data)
    ...
    public func peripheral(_ peripheral: CBPeripheral, didUpdateNotificationStateFor characteristic: CBCharacteristic, error: Error?) {
        ...
        if (characteristic.uuid.uuidString == PROBUFF_CHARACT_DFU_UUID) {
            DispatchQueue.main.asyncAfter(deadline: .now() + 3) {
                CmdTool.sharedInstance.writeUpgradeCmdA(peripheral)
            }
    ...
    public func peripheral(_ peripheral: CBPeripheral, didWriteValueFor characteristic: CBCharacteristic, error: Error?) {
        if (characteristic.uuid.uuidString == PROBUFF_CHARACT_DFU_UUID) {
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.2) {
                CmdTool.sharedInstance.writeUpgradeCmdB(peripheral)
            }
